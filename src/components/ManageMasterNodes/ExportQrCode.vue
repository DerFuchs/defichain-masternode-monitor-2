			...
			<q-card-section class="q-px-none q-py-none">
				...
				>
					...
					<q-card :class="{ 'bg-grey-2': basics.darkMode === false }">
						<q-card-section>
							<p>
								Bring your watched masternodes to another Masternode Monitor,
			...
			<q-card-section>
				...
				>
					<q-dialog
						...
						persistent
						:maximized="user.screen.lt.sm"
						transition-show="slide-up"
						transition-hide="slide-down"
					>
						<q-card>
							<q-card-section class="bg-primary-dark row items-center q-py-sm">
								<div class="text-h6 text-white">Export</div>
								<q-space />
								<q-btn
									v-close-popup
									flat
                  ...
                  -->
							...
							<q-card-section class="text-body2 text-weight-light">
								Scan this image using the "import" button in the "Manage
								Masternodes" section in another Masternode Monitor to import
								this Masternode Monitor's watched masternodes.
							</q-card-section>
							<q-card-section class="text-body2 text-weight-light q-mb-xl">
								You can also save this image to save your masternode list.
							</q-card-section>

							<q-card-section v-if="user.screen.lt.sm" class="fixed-bottom">
								<q-btn
									v-close-popup
									outline
									icon="fa-light fa-close"
				...
				>
					...
					>
						<q-card>
							<q-card-section class="bg-primary-dark row items-center q-py-sm">
								<div class="text-h6 text-white">Import</div>
								<q-space />
								<q-btn
									v-close-popup
									flat
							...
							>
								Scan the QR code showing by tapping on the "export" button in
								the "Manage Masternodes" section in another Masternode Monitor
								to import that Masternode Monitor's watched masternodes into
								this one.
							</q-card-section>
							...
							<q-card-section v-if="hasMasternodesToBeImported">
								<q-list bordered separator>
									<q-item
										v-for="(
											masternodeName, ownerAddress
										) in importMasternodeList"
										:key="ownerAddress"
									>
										<q-item-section>
											<q-item-label>{{
												masternodeName || "Unnamed"
											}}</q-item-label>
...
export default defineComponent({
	...
	setup() {
		...
		const importAdd = async () => {
			for await (const [
				masternodeOwnerAddress,
				masternodeName,
			] of Object.entries(importMasternodeList.value)) {
