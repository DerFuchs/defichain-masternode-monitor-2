<template>
  <settings-card headline="Custom Ocean Data Source">
    <template #description>
      ...
      <a class="text-primary" href="https://github.com/defichain-pond/pond-deploy"
        >@sandrich's Defichain Pond</a
      >
      or just using another one? Paste its address in here and Masternode Monitor will use
      it as it's primary data source.
    </template>
    <q-card-section>
      <q-input
        v-model="newOceanUrl"
...
export default defineComponent({
  ...
  setup() {
    ...
    const newOceanUrl = ref(JSON.parse(JSON.stringify(user.settings.customOceanUrl))); // that's a dirty trick to remove the reference to the setting and treat it just like a local string
