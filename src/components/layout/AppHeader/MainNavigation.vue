<template>
	<q-drawer side="left" behavior="mobile">
		<q-list>
			<q-item-label header>
				<q-img
					src="icons/master-node-monitor-logo.svg"
					height="40px"
					fit="contain"
				/>
			</q-item-label>
			...
			<div class="row justify-evenly">
				...
				>
					<q-item-section class="text-center">
						<q-item-label>
							<q-icon
								name="fa-light fa-screwdriver-wrench"
								size="xs"
								class="q-mb-xs"
							/>
						</q-item-label>
				...
				<q-item clickable tag="a" class="col-3 flex flex-center">
					<q-item-section class="text-center">
						<q-item-label>
							<q-icon
								name="fa-light fa-brackets-curly"
								size="xs"
								class="q-mb-xs"
							/>
						</q-item-label>
...
export default defineComponent({
	...
	setup() {
		...
		const lastRefreshAgo = ref(
			DateTime.fromISO(basics.lastRefresh).toRelative()
		);

		onMounted(() => {
			const refreshTimer = setInterval(() => {
				lastRefreshAgo.value = DateTime.fromISO(
					basics.lastRefresh
				).toRelative();
			}, 10000);
		});
